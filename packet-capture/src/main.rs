...
fn main() {
    ...
    let interface = interfaces
        ...
        .expect("Failed to get inrerface");
...
// Display applciation layer data as binary
fn print_packet_info(rawPacket: &GettableEndPoints, packet: &GettableEndPoints, protocol: &str) {
    println!(
        "Captured a {} packet from {}|{} to {}|{}\n",
        protocol,
        rawPacket.get_source(),
        packet.get_source(),
        rawPacket.get_destination(),
        packet.get_destination()
    );
    let payload = packet.get_payload();
    let len = payload.len();

    // display payload within constraint sizes
    for i in 0..len {
        print!("{:<02X}", payload[i]);
